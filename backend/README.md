...
**Autenticación (agregada 2026-07-26, JWT, sin roles):** todos los endpoints requieren `Authorization: Bearer <token>` excepto `POST /auth/login` y `GET /materiales` (el listado público de precios que usa la app móvil sin login, ver `app-movil/src/screens/HomeScreen.tsx`). Se aplica a nivel de router con `dependencies=[Depends(get_current_user)]` (`app.core.security`); `materiales` es el único módulo mixto, así que protege sus rutas de admin una por una en vez de a nivel de router. No hay roles/permisos distintos por usuario — cualquier cuenta autenticada puede todo (incluida la gestión de otras cuentas, ver `/usuarios` abajo); la única razón de tener varios usuarios (tabla `usuarios`, antes `admin`) es trazabilidad (`creado_por`/`usuario_id`, ver `../base-datos/README.md`), no control de acceso.
...
**Gestión de usuarios (agregado 2026-07-26, `/usuarios`, requiere auth):**
- `GET /usuarios`, `GET /usuarios/{id}` — sin `password_hash` en la respuesta.
- `POST /usuarios` — `{"usuario", "password"}` (`password` mínimo 8 caracteres, 422 si no). 409 si `usuario` ya existe.
- `PATCH /usuarios/{id}` — `{"activo": false}` y/o `{"password": "..."}`, al menos uno de los dos (400 si ninguno). No hay `DELETE` -- dar de baja es `activo: false`, igual que proveedores/clientes/materiales.
